        ...
        <table class="tree-table-header" style="width: 100%">
          ...
          <tbody style="visibility: collapse">
            <tr class="tree-table-row">
              <td v-for="column in finalColumnNum" :key="column" :rowspan="1" :colspan="1"></td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <table ref="tableBody" class="tree-table-body" style="width: 100%">
          ...
          <tbody>
            <tr v-for="(tdList, index) in trList" :key="index" class="tree-table-row">
              ...
              >
                <div class="td-container">
                  <div v-if="tdData['showFold'] && isFold" @click="onFold(tdData)">{{
                    tdData['isFold'] ? '-' : '+'
                  }}</div>
                  ...
                  <template v-if="showCheckbox">
                    <Checkbox
                      ...
                      v-model:checked="tdData['isChecked']"
                      class="td-container-checkbox"
                      :disabled="
                        tdData['editable'] === falseEditLabel ||
                        (disabledCheckboxNodeLevel
                          ? tdData['nodeLevel'] <= disabledCheckboxNodeLevel
                          : false) ||
                        (disabledCheckboxLevels
                          ? disabledCheckboxLevels.includes(tdData['nodeLevel'])
                          : false) ||
                        (disabledCheckboxIds ? disabledCheckboxIds.includes(tdData['id']) : false)
                      "
                      :true-label="trueCheckLabel"
                      :false-label="falseCheckLabel"
                      @change="(e) => onCheck(e, tdData)"
                    >
                      {{ tdData['nodeName'] }}
                    </Checkbox>
                    <slot
            ...
            <tr style="visibility: collapse">
              <td v-for="level of finalColumnNum" :key="level"></td>
            </tr>
          </tbody>
        </table>
      </div>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    watch: {
      originData: {
        handler(_newVal, _oldVal) {
          // 格式化json
          this.treeData = JSON.parse(JSON.stringify(_newVal));
          // 映射map
      ...
      tableColumns: {
        handler(_newVal, _oldVal) {
          const tableColumns = JSON.parse(JSON.stringify(_newVal));
          if (tableColumns.length > 0) {
            const tableColumnsItem = this.tableColumns[0] as Array<{ colSpan: any }>;
            this.maxColumnNum = tableColumnsItem.reduce((acc, cur) => {
    ...
    mounted() {
      ...
      // 创建观察者对象
      const observer = new MutationObserver((_mutationmutationList, _observers) => {
      ...
      // 传入目标节点和观察选项
      observer.observe(this.$refs.tableBody as Node, config);
      // 随后,你还可以停止观察
      // observer.disconnect()
      this.$nextTick(() => {
    ...
    methods: {
      ...
      checkHasScrollerbar() {
        ...
        const tableHeaderWrapper = this.$refs.tableHeaderWrapper as HTMLElement;
        this.isShowScrollbar =
          tableBody.offsetHeight > tableWrapper.offsetHeight - tableHeaderWrapper.offsetHeight;
      ...
      onCheck(e, node) {
        ...
        const values = this.getCheckedNodeList().map((item) => item['id']);
        console.log(values);
      ...
      // 获取额外列的节点数据
      getExtraNodeList() {
        const extraNodeList = [] as any[];
        const trList = this.trList as any[][];
        trList.forEach((e) => {
